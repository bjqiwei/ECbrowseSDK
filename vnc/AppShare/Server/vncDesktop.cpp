...
unsigned vncDesktop::CheckUpdatesThread(LPVOID lpVoid)
{
	vncDesktop *pServer = (vncDesktop *)lpVoid;
	if (pServer == NULL) return 0;
	int count = 0;
	int ret = 0;
	int wait = pServer->m_server->GetCaptureInterval();
	while(WaitForSingleObject(pServer->m_hKillClient,wait) != WAIT_OBJECT_0)
	{
		try
		{
			if (pServer->m_server->WindowShared())
			{
				if (!::IsWindowVisible(pServer->m_server->GetWindowShared()))
					continue;
			}
			ret = 0;
			if (!pServer->CheckUpdates(ret)) 
			{
				pServer->m_server->DisplayChange();
				break;
			}
			if (ret)
			{
				pServer->m_server->CaptureKeyFrame();
			}
			wait = pServer->m_server->GetCaptureInterval();
		}
		catch (...)
		{
			;
...
vncDesktop::vncDesktop(BOOL bRecordScreen)
{
...
vncDesktop::~vncDesktop()
{
	...
	if (m_hTryThread)
	{
		SetEvent(m_hKillClient);
		WaitForSingleObject(m_hTryThread,5000);
		CloseHandle(m_hTryThread);
		CloseHandle(m_hKillClient);
		m_hKillClient	= NULL;
		m_hTryThread			= NULL;
	}
...
// FIXME: MessageBox in a service
	...
	if ((GetDeviceCaps(m_hrootdc, RASTERCAPS) & RC_DI_BITMAP) == 0)
	{
// FIXME: MessageBox in a service
		MessageBox(
			NULL,
	...
	result = ::GetDIBits(m_hrootdc, m_membitmap, 0, 1, NULL, &m_bminfo.bmi, DIB_RGB_COLORS);
	if (result == 0) {
		return FALSE;
	}
	result = ::GetDIBits(m_hrootdc, m_membitmap, 0, 1, NULL, &m_bminfo.bmi, DIB_RGB_COLORS);
	...
	if (GetDeviceCaps(m_hrootdc, PLANES) != 1)
	{
// FIXME: MessageBox in a service
		MessageBox(
			NULL,
...
vncDesktop::SetPalette()
{
	...
	if (!m_bminfo.truecolour)
	{
		...
		HPALETTE oldpalette = SelectPalette(m_hrootdc, pal, FALSE);
		if (oldpalette == NULL)
		{
		...
		if (RealizePalette(m_hrootdc) == GDI_ERROR)
...
vncDesktop::InitWindow()
{
	if (m_wndClass == 0) {
		...
		wndclass.hInstance		= NULL;//hAppInstance;
		wndclass.hIcon			= NULL;
...
vncDesktop::Init(vncServer *server)
{
	...
	m_hKillClient	= CreateEvent(NULL, TRUE, FALSE, NULL);
	if(m_hKillClient == INVALID_HANDLE_VALUE)
	{
		
		return FALSE;
	}
	UINT	dwThreadId = 0;
	m_hTryThread =(HANDLE)_beginthreadex(NULL,				// Security
		0,					// Stack size - use default
		CheckUpdatesThread,  // Thread fn entry point
		(void*) this,	    
		0,					// Init flag
		&dwThreadId);	// Thread address
	if(m_hTryThread == INVALID_HANDLE_VALUE)
	{
		CloseHandle(m_hKillClient);
		return FALSE;
	}
...
//	GdiFlush();
	...
	HDC hmemdc = CreateCompatibleDC(m_hrootdc);

	// Select the memory bitmap into the memory DC
	HBITMAP oldbitmap;
	if ((oldbitmap = (HBITMAP) SelectObject(hmemdc, m_membitmap)) == NULL)
		return;
	...
	if (!GetVersionEx(&osversioninfo))
		return;
	bool isp = osversioninfo.dwPlatformId==VER_PLATFORM_WIN32_NT;

	// Capture screen into bitmap
	BOOL blitok =::BitBlt(hmemdc, rect.left - m_bmrect.left,
		rect.top - m_bmrect.top,
		rect.right - rect.left,
		rect.bottom - rect.top,
		m_hrootdc,
		rect.left, rect.top,
		isp ? CAPTUREBLT | SRCCOPY : SRCCOPY);
...
void vncDesktop::CaptureMouse(BYTE *scrBuff, UINT scrBuffSize)
{
	...
	// Capture screen into bitmap
	BOOL blitok =::BitBlt(hmemdc, m_cursorpos.left - m_bmrect.left,
		m_cursorpos.top - m_bmrect.top,
		m_cursorpos.right - m_cursorpos.left,
		m_cursorpos.bottom - m_cursorpos.top,
		m_hrootdc,
		m_cursorpos.left, m_cursorpos.top,
		isp ? CAPTUREBLT | SRCCOPY : SRCCOPY);
...
vncDesktop::GetRichCursorData(BYTE *databuf, HCURSOR hcursor, int width, int height)
{
	...
	HBITMAP oldbitmap = (HBITMAP) SelectObject(hmemdc, membitmap);
	if (oldbitmap == NULL) {
	...
	// Draw the cursor
	DrawIconEx(hmemdc, 0, 0, hcursor, 0, 0, 0, NULL, DI_IMAGE);
	SelectObject(hmemdc, oldbitmap);
	...
	int lines = GetDIBits(hmemdc, membitmap, 0, height, databuf, bmi, DIB_RGB_COLORS);
...
// NB: m_membitmap is bottom2top
	...
	else
	{
		...
		//	memcpy(destbuffpos, srcbuffpos, widthBytes);
			CopyMemory(destbuffpos, srcbuffpos, widthBytes);
...
BOOL vncDesktop::CheckUpdates(int &ret)
{
#ifndef _DEBUG
	try
	{
...
// window is resized
	...
	// IMPORTANT: this order: CopyRectToBuffer, CaptureScreen, GetChangedRegion
			...
			if (m_server->FullRgnRequested() || m_server->IncrRgnRequested())
			{
				...
				// 增加这一句，共享动态的ppt效果就好些,发送的数据会多5%左右,不知道为什么 -wkh
...
inline void vncDesktop::CheckRects(vncRegion &rgn, rectlist &rects)
{
	...
	try
	{
		...
		for (i = rects.begin(); i != rects.end(); i++)
		{
			...
			if (r.top < 0) r.top = 0;
			CaptureScreen(r, m_mainbuff);

// Check for changes in the rectangle
			GetChangedRegion(rgn, r);
		}
#ifndef _DEBUG
	}
	catch (...)
