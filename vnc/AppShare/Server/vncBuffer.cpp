...
rfbPixelFormat vncBuffer::GetAgentFormat()
{
	if (m_clientfmtset)
		return m_clientformat;
...
BYTE *vncBuffer::GetMainBuffer()
{
	omni_mutex_lock l(m_clientsLock);
	return m_mainbuff;
}
int  vncBuffer::GetMainBufferSize()
{
	return m_mainsize;
}
BOOL
vncBuffer::CheckBuffer()
{
	...
	if (m_encoder == NULL) {
	    if (!SetEncoding(rfbEncodingTight))//rfbEncodingZlib rfbEncodingRaw
		return FALSE;
	}
...
vncBuffer::SetQualityLevel(int level)
{
	...
	{
		m_qualitylevel = 0;
		rfbPixelFormat vnc8Format = {8, 8, 0, 1, 7,7,3, 0,3,6,0,0};
		SetClientFormat(vnc8Format);
		return ;
	}
	SetClientFormat(m_scrinfo.format);
...
//coder
int vncBuffer::GetQualityLevel(void)
{
