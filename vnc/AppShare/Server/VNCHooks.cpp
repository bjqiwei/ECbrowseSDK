...
KillPropsProc(HWND hwnd, LPARAM lParam)
{
	...
	RemoveProp(hwnd, (LPCTSTR) MAKEWORD(VNC_WINDOWPOS_ATOM2, 0));
...
#pragma message("warning:Low-Level Keyboard hook code omitted.")
 ...
 BOOL SetKeyboardPriorityLLHook(HWND hwnd, BOOL activate, UINT LocalKbdMsg)
{
...
#pragma message("warning:Low-Level Mouse hook code omitted.")
 ...
 BOOL SetMousePriorityLLHook(HWND hwnd, BOOL activate, UINT LocalMouseMsg)
{
...
// Routine to start and stop local keyboard message filtering
 BOOL SetKeyboardPriorityHook(HWND hwnd, BOOL activate, UINT LocalKbdMsg)
{
	LocalKeyboardMessage = LocalKbdMsg;
	if (activate)
	{
...
// Routine to start and stop local mouse message filtering
 BOOL SetMousePriorityHook(HWND hwnd, BOOL activate, UINT LocalMouseMsg)
{
	LocalMouseMessage = LocalMouseMsg;
	if (activate)
	{
