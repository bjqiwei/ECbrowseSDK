...
void ClientConnection::ReadTightRect(rfbFramebufferUpdateRectHeader *pfburh)
{
  ...
  CARD8 comp_ctl;
  if (!ReadExact((char *)&comp_ctl, 1)) return;
  ...
  if (comp_ctl == rfbTightFill) {
    ...
    if (m_myFormat.depth == 24 && m_myFormat.redMax == 0xFF &&
      ...
      if (!ReadExact((char *)&fillColourBuf, 3)) return;
      fillColour = COLOR_FROM_PIXEL24_ADDRESS(fillColourBuf);
    } else {
      CARD32 fillColourBuf;
      if (!ReadExact((char *)&fillColourBuf, m_myFormat.bitsPerPixel / 8)) return;
  ...
  if ((comp_ctl & rfbTightExplicitFilter) != 0) {
    CARD8 filter_id;
    if (!ReadExact((char *)&filter_id, 1)) return ;
  ...
  if (pfburh->r.h * rowSize < TIGHT_MIN_TO_COMPRESS) {
    ...
    if (!ReadExact(m_netbuf, pfburh->r.h * rowSize)) return;
  ...
  z_streamp zs = &m_tightZlibStream[stream_id];
	...
	{
		inflateEnd(&m_tightZlibStream[stream_id]);
		m_tightZlibStreamActive[stream_id] = false;
	}
	//end
  if (!m_tightZlibStreamActive[stream_id]) {
    zs->zalloc = Z_NULL;
  ...
  while (compressedLen > 0) {
    ...
    if (!ReadExact(m_tightbuf, portionLen)) return;
